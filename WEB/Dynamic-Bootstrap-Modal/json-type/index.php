	...
	<div class="container">
        ...
        <div class="row">
        	...
        	<div class="col-lg-12">
            		...
            		$query = "SELECT * FROM nachosmenu";
            		$stmt = $DBcon->prepare( $query );
            		$stmt->execute();
            		while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
						?>
						<tr>
						<td><?php echo $row['prod_name']."&nbsp;".$row['price']; ?></td>
						<td>
						<button data-toggle="modal" data-target="#view-modal" data-id="<?php echo $row['product_id']; ?>" id="getUser" class="btn btn-sm btn-info"><i class="glyphicon glyphicon-eye-open"></i> View</button>
						</td>
						</tr>
						<?php
...
$(document).ready(function(){
	...
	$(document).on('click', '#getUser', function(e){
		...
		.done(function(data){
			...
			$('#prod_number').html(data.product_id);
			$('#txt_lname').html(data.prod_name);
			$('#txt_email').html(data.price);
			$('#txt_position').html(data.category);
			$('#txt_office').html(data.description);
